...
router.post("/:idPost/:idReceptor", async (req, res) => {
  ...
  msg = msg ? msg : "";
  let mensaje = { idSenderUser: req._id, message: msg, date: Date.now() };
...
router.delete("/:idMessage", async (req, res) => {
  ...
  if (msgToRemove) {
    let removeMsg = await Chat.findOneAndUpdate(
      ...
      {
        $pull: {
          messages: { _id: mongoose.Types.ObjectId(req.params.idMessage) },
        },
      },
      { new: true }
    );
